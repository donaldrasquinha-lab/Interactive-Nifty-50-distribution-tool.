...
# Custom light styling blocks for scannability
st.markdown("""
<style>
@import url('https://googleapis.com');

/* Main Canvas Foundations */
...
.direction-card {
    ...
    border: 1px solid #e2e8f0; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.05);
}
.score-label { font-size: 11px; letter-spacing: 2px; color: #f8fafc; opacity: 0.85; margin-bottom: 4px; }
.direction-text { font-size: 26px; font-weight: 700; color: #ffffff; }
.sentiment-text { font-size: 13px; color: #f1f5f9; margin-top: 4px; }
...
/* Hide standard dashboard user interface anchors */
...
card_bg_color = "#16a34a" if direction_label == "BULLISH" else ("#dc2626" if direction_label == "BEARISH" else "#475569")

# Render Clean Metric Panels
...
<div class="direction-card" style="background: {card_bg_color}; border-color: {card_bg_color};">
    ...
    <div class="sentiment-text">Calculated using live Put-Call buildup profiles, key open interest walls, and normal price distribution variations.</div>
</div>
""", unsafe_allow_html=True)
...
# Render White-themed Grid Graphs Side-by-Side
...
with col_left:
    ...
    ax_p.fill_between(x, prob_density, 0, where=(x >= sd1_lower) & (x <= sd1_upper), facecolor='#10b981', alpha=0.2, label='68.2% Confidence Zone (1 SD)')
    ax_p.fill_between(x, prob_density, 0, where=((x >= sd2_lower) & (x < sd1_lower)) | ((x > sd1_upper) & (x <= sd2_upper)), facecolor='#f59e0b', alpha=0.12, label='95.4% Confidence Zone (2 SD)')
    
    ax_p.axvline(spot_price, color='#4f46e5', linestyle=':', linewidth=1.5, label=f'Spot Price Level')
    ax_p.axvline(strike_sell, color='#e11d48', linestyle='--', linewidth=1.2, label=f'OI Strike Barrier')
...
with col_right:
    ...
    if show_adjustment:
        ...
        ax_t.scatter(max_prof_x, max_prof_y, color='#1d4ed8', s=80, zorder=5)
    else:
